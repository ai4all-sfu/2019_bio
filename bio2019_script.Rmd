...
#save(PreRF,file=paste0(input_dir,"/script.Rdata"))


feat.ra = data0[,order(varImp(PreRF.i)$importance$Overall, decreasing=T)[1:500]]
...
#----------------------   ensemble model 
# train models; note some are classification models 
models_reg1 = c(
  "enet", # 8.5
  "glmnet", # 8.5
  "kernelpls", # 8.5
...
#Training model
model_list = caretList( y=ga_, x=train_[,-1], trControl=fitcv, metric="RMSE", methodList=models_reg1, continue_on_fail=T)
# weighted linear combination of model predictions
ensemble1 = caretEnsemble( model_list, metric="RMSE",  trControl=fitcv)
summary(ensemble1)

predictions_e1 = predict(ensemble1, newdata = train_[,-1])
predictions_e1[predictions_e1<8] = 8
predictions_e1[predictions_e1>42] = 42
rmse(ga_, predictions_e1)

#PLOT 
plotdata = data.frame(ga_,predictions_e1)
ggplot(plotdata, aes(x = ga_,y = predictions_e1))+
  geom_point(color = 'lightgreen')+
  xlab('Ensemble - Training Set')+
  ylab('Predicted Values')

```

### Exercise 9: 
Run another Ensemble. This time you are the responsable for picking a few models from the list of extra models to ensemble 

```{r}
#----------------------   ensemble model 
# train models; note some are classification models 
models_reg2 = c()#ADD YOUR MODELS HERE#


#Training model
model_list = caretList( y=ga_, x=train_[,-1], trControl=fitcv, metric="RMSE", methodList=models_reg2, continue_on_fail=T)
# weighted linear combination of model predictions
ensemble2 = caretEnsemble( model_list, metric="RMSE",  trControl=fitcv)
summary(ensemble2)

predictions_e2 = predict(ensemble2, newdata = train_[,-1])
predictions_e2[predictions_e2<8] = 8
predictions_e2[predictions_e2>42] = 42
rmse(ga_, predictions_e2)

#PLOT 
plotdata = data.frame(ga_,predictions_e2)
ggplot(plotdata, aes(x = ga_,y = predictions_e2))+
  geom_point(color = 'lightgreen')+
  xlab('Ensemble - Training Set')+
  ylab('Predicted Values')
```
